...
def compute_influence_with_ekfac(
    ...
    return results, ihvp_calculator
...
def compute_tokenwise_influence_pytorch(
    pt_model,
    tokenizer,
    x_train_text: str,
    y_train: int,
    ...
    """
    Calcule l'influence par token en utilisant PyTorch directement.

    Cette fonction calcule:
    influence_token = d/d(input_embeddings) [ihvp^T @ grad_theta L(x_train, y_train)]

    Returns
    -------
    influence_per_token : np.ndarray
        Influence par token de shape (seq_len,)
    tokens : list
        Liste des tokens
    attention_mask : np.ndarray
        Masque d'attention
    total_influence : float
        Influence totale (somme)
    """
    pt_model.eval()

    # Tokeniser l'exemple d'entrainement
    enc_train = tokenizer(
        x_train_text,
        truncation=True,
        padding="max_length",
    ...
    input_ids_train = enc_train["input_ids"].to(DEVICE)
    attention_mask_train = enc_train["attention_mask"].to(DEVICE)

    tokens = tokenizer.convert_ids_to_tokens(input_ids_train[0])

    # Convertir IHVP en tensor PyTorch
    ihvp_tensor = torch.tensor(ihvp_vec, device=DEVICE, dtype=torch.float32)

    # Obtenir les embeddings d'entree et activer les gradients
    if hasattr(pt_model, 'model') and hasattr(pt_model.model, 'embeddings'):
        embeddings_layer = pt_model.model.embeddings
    else:
        embeddings_layer = pt_model.get_input_embeddings()

    # Obtenir les embeddings
    input_embeds = embeddings_layer(input_ids_train)
    input_embeds.requires_grad_(True)

    # Forward pass avec embeddings au lieu d'input_ids
    outputs = pt_model.model(
    ...
    # Classifier
    logits = pt_model.classifier(pooled)
    ...
    # Gradient par rapport aux parametres du classifier uniquement
    classifier_params = list(pt_model.classifier.parameters())
    ...
    # Dot product avec IHVP
    influence_scalar = torch.dot(ihvp_tensor, grad_vec)

    # Backprop vers les embeddings d'entree
    influence_grad = torch.autograd.grad(
        influence_scalar,
        input_embeds,
        retain_graph=False
    )[0]  # Shape: (1, seq_len, hidden_dim)

    # Norme L2 par token pour obtenir l'influence par token
    influence_per_token = torch.norm(influence_grad[0], dim=-1).cpu().numpy()

    # Influence totale
    total_influence = influence_scalar.item()

    return influence_per_token, tokens, attention_mask_train[0].cpu().numpy(), total_influence


def display_results_with_tokenwise(
    ...
    """Affiche les resultats d'influence avec analyse tokenwise."""
    print(f"\n{'='*80}")
    print("RESULTATS D'INFLUENCE TOKENWISE (EKFAC - Transformer)")
    print(f"{'='*80}")
    ...
    for batch_idx, test_lab_batch in enumerate(results["test_labels"]):
        ...
        for i in range(len(test_lab_batch)):
            ...
            # Calculer l'IHVP pour cet exemple de test
            ...
            ihvp = ihvp_calculator._compute_ihvp_single_batch(test_batch)
            ...
            for rank in range(min(top_k, len(inf_vals_pos))):
                ...
                print(f"  Texte: {train_example['text'][:150]}...")

                # Calculer l'influence par token
                try:
                    influence_per_token, tokens, mask, total_inf = compute_tokenwise_influence_pytorch(
                        pt_model, tokenizer,
                        train_example["text"],
                    ...
                    # Filtrer les tokens valides (non-padding)
                    valid_indices = mask == 1
                    valid_tokens = [tokens[j] for j in range(len(tokens)) if valid_indices[j]]
                    valid_scores = influence_per_token[valid_indices]

                    if len(valid_tokens) > 0:
                        top_token_idx = np.argsort(valid_scores)[-5:][::-1]
                        print("  Tokens les plus influents:")
                        for ti in top_token_idx:
                            print(f"    - '{valid_tokens[ti]}': {valid_scores[ti]:.4f}")
...
def main():
    ...
    if INFLUENCE_MODE == "classifier_only":
        ...
        results, ihvp_calculator = compute_influence_with_ekfac(
        ...
        # Afficher les resultats avec tokenwise
        display_results_with_tokenwise(
            results, train_ds_hf, test_ds_hf, train_embeddings,
            test_embeddings, test_labels, label_names, label2id,
            pt_model, tokenizer, ihvp_calculator, MAX_LEN, top_k=TOP_K
        )
