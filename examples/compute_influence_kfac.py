...
from deel.influenciae.common import InfluenceModel, KFACIHVP
...
def compute_influence_with_influenciae(
    ...
    """
    Calcule les scores d'influence en utilisant influenciae avec KFAC.
    """
    print(f"\n{'='*60}")
    print("Calcul d'influence avec influenciae (KFAC)")
    print(f"{'='*60}")
    ...
    # Calculateur IHVP avec KFAC (Kronecker-Factored Approximate Curvature)
    print("Création du calculateur IHVP (KFAC)...")
    ...
    for layer_idx, (A, G) in ihvp_calculator.kfac_blocks.items():
        ...
        print(f"  A has NaN: {tf.reduce_any(tf.math.is_nan(A)).numpy()}, G has NaN: {tf.reduce_any(tf.math.is_nan(G)).numpy()}")
...
def display_influence_results(
    ...
    """
    print(f"\n{'='*80}")
    print("RÉSULTATS D'INFLUENCE (KFAC)")
    print(f"{'='*80}")
...
def main():
    ...
    print("Méthode: KFAC (Kronecker-Factored Approximate Curvature)")
    print("="*60)
